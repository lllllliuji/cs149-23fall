...
void clampedExpVector(float* values, int* exponents, float* output, int N) {
    ...
    __cs149_vec_int exp;
    const float limit = 9.999999f;
    __cs149_vec_float maxx = _cs149_vset_float(limit);
    __cs149_vec_int one = _cs149_vset_int(1);
    __cs149_vec_int zero = _cs149_vset_int(0);

    for (int i = 0; i < N; i += VECTOR_WIDTH) {
        // init data width
        __cs149_mask mask = _cs149_init_ones(min(VECTOR_WIDTH, N - i));

        // init result
        _cs149_vset_float(result, 1.f, mask);

        // load operator number
        _cs149_vload_float(base, values + i, mask);
        _cs149_vload_int(exp, exponents + i, mask);

        // compute non-zero exponents
        __cs149_mask maskPositive = _cs149_init_ones(0);
        _cs149_vgt_int(maskPositive, exp, zero, mask);

        while (_cs149_cntbits(maskPositive) > 0) {
            // do the compute
            _cs149_vmult_float(result, result, base, maskPositive);
            // update exp
            _cs149_vsub_int(exp, exp, one, maskPositive);
            _cs149_vgt_int(maskPositive, exp, zero, mask);
        }
        // clamps the resulting value at 9.999999
        __cs149_mask exceedLimit = _cs149_init_ones(0);
        _cs149_vgt_float(exceedLimit, result, maxx, mask);
        _cs149_vset_float(result, limit, exceedLimit);
...
float arraySumVector(float* values, int N) {
    ...
    __cs149_vec_float sum_vec;
    _cs149_vset_float(sum_vec, 0.f, maskAll);
    for (int i = 0; i < N; i += VECTOR_WIDTH) {
        // init data width
        __cs149_mask mask = _cs149_init_ones(min(VECTOR_WIDTH, N - i));
        _cs149_vload_float(data, values + i, mask);
        _cs149_vadd_float(sum_vec, sum_vec, data, mask);
    }
    float sum = 0.f;
